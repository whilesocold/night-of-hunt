...
export class NavMenuItem extends PIXI.Container {
...
export class NavMenu extends PIXI.Container {
  protected items: NavMenuItem[]
  protected item: NavMenuItem
  ...
  constructor(items: { texture: string, title: string }[]) {
    super()
    ...
    items.forEach(item => this.addItem(item.texture, item.title))
  }

  protected addItem(texture: string, title: string): NavMenuItem {
    const previousItem = this.items.length > 0 ? this.items[this.items.length - 1] : null

    const item = new NavMenuItem(texture, title)
